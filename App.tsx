...
import { AppState, WalletType, Wallet, Department, Account, Subscription, TransactionType, Transaction, EntityStatus } from './types';
import { LayoutDashboard, WalletCards, List, Users, BookOpen, FileText, LogOut } from 'lucide-react';
...
function App() {
  ...
  useEffect(() => {
    localStorage.setItem('subtrack_state', JSON.stringify(state));
  }, [state]);
  ...
  const fundMainWallet = (amount: number) => {
    setState(prev => {
      ...
      return { ...prev, wallets: updatedWallets, transactions: [...prev.transactions, transaction] };
    });
  };
  ...
  const editTransaction = (txId: string, updates: Partial<Transaction>) => {
    setState(prev => {
      ...
      if (oldTx.type === TransactionType.DEPOSIT_FROM_BANK && oldTx.toWalletId) {
         ...
         if (wIndex > -1) tempWallets[wIndex] = { ...tempWallets[wIndex], balance: tempWallets[wIndex].balance - oldTx.amount };
      } else if (oldTx.type === TransactionType.INTERNAL_TRANSFER && oldTx.fromWalletId && oldTx.toWalletId) {
         const fromIdx = tempWallets.findIndex(w => w.id === oldTx.fromWalletId);
         const toIdx = tempWallets.findIndex(w => w.id === oldTx.toWalletId);
         if (fromIdx > -1) tempWallets[fromIdx] = { ...tempWallets[fromIdx], balance: tempWallets[fromIdx].balance + oldTx.amount };
         if (toIdx > -1) tempWallets[toIdx] = { ...tempWallets[toIdx], balance: tempWallets[toIdx].balance - oldTx.amount };
      } else if (oldTx.type === TransactionType.SUBSCRIPTION_PAYMENT && oldTx.fromWalletId) {
         const wIndex = tempWallets.findIndex(w => w.id === oldTx.fromWalletId);
         if (wIndex > -1) tempWallets[wIndex] = { ...tempWallets[wIndex], balance: tempWallets[wIndex].balance + oldTx.amount };
      } else if (oldTx.type === TransactionType.REFUND && oldTx.toWalletId) {
         // Revert Refund (Remove balance)
         const wIndex = tempWallets.findIndex(w => w.id === oldTx.toWalletId);
         if (wIndex > -1) tempWallets[wIndex] = { ...tempWallets[wIndex], balance: tempWallets[wIndex].balance - oldTx.amount };
      }
      ...
      if (updatedTx.type === TransactionType.DEPOSIT_FROM_BANK && updatedTx.toWalletId) {
         ...
         if (wIndex > -1) tempWallets[wIndex] = { ...tempWallets[wIndex], balance: tempWallets[wIndex].balance + newAmount };
      } else if (updatedTx.type === TransactionType.INTERNAL_TRANSFER && updatedTx.fromWalletId && updatedTx.toWalletId) {
         ...
         if (fromIdx > -1) tempWallets[fromIdx] = { ...tempWallets[fromIdx], balance: tempWallets[fromIdx].balance - newAmount };
         if (toIdx > -1) tempWallets[toIdx] = { ...tempWallets[toIdx], balance: tempWallets[toIdx].balance + newAmount };
      } else if (updatedTx.type === TransactionType.SUBSCRIPTION_PAYMENT && updatedTx.fromWalletId) {
  ...
  const deleteTransaction = (txId: string) => {
    setState(prev => {
      ...
      if (tx.type === TransactionType.SUBSCRIPTION_PAYMENT && tx.fromWalletId) {
         ...
         if (wIndex > -1) tempWallets[wIndex] = { ...tempWallets[wIndex], balance: tempWallets[wIndex].balance + tx.amount };
      } else if (tx.type === TransactionType.DEPOSIT_FROM_BANK && tx.toWalletId) {
          const wIndex = tempWallets.findIndex(w => w.id === tx.toWalletId);
          // Revert deposit: Remove funds
          if (wIndex > -1) tempWallets[wIndex] = { ...tempWallets[wIndex], balance: tempWallets[wIndex].balance - tx.amount };
      } else if (tx.type === TransactionType.INTERNAL_TRANSFER && tx.fromWalletId && tx.toWalletId) {
          ...
          if (fromIdx > -1) tempWallets[fromIdx] = { ...tempWallets[fromIdx], balance: tempWallets[fromIdx].balance + tx.amount };
          if (toIdx > -1) tempWallets[toIdx] = { ...tempWallets[toIdx], balance: tempWallets[toIdx].balance - tx.amount };
      } else if (tx.type === TransactionType.REFUND && tx.toWalletId) {
          // Revert Refund: Remove added balance
          const wIndex = tempWallets.findIndex(w => w.id === tx.toWalletId);
          if (wIndex > -1) tempWallets[wIndex] = { ...tempWallets[wIndex], balance: tempWallets[wIndex].balance - tx.amount };
      }
  ...
  // Record an actual payment against a service
  const recordPayment = (subscriptionId: string, walletId: string, amount: number, date: string, nextRenewalDate: string, vatAmount?: number) => {
    setState(prev => {
      ...
      // Validation check - prevent negative balance transactions
      ...
      const updatedSubs = prev.subscriptions.map(s => {
        if (s.id === subscriptionId) {
          return { 
            ...
            nextRenewalDate: nextRenewalDate // Use the user provided date
          };
        }
        return s;
      });
  ...
  const recordRefund = (subscriptionId: string, walletId: string, amount: number, date: string) => {
    setState(prev => {
      ...
      const subName = prev.subscriptions.find(s => s.id === subscriptionId)?.name || 'Service';
      ...
      const refundTx: Transaction = {
        ...
        toWalletId: walletId, // Money goes back TO the wallet
        subscriptionId: subscriptionId,
  ...
  // --- Auth Gate ---
  if (isAuthChecking) {
     return <div className="min-h-screen flex items-center justify-center bg-gray-50"><div className="animate-spin h-8 w-8 border-4 border-indigo-500 rounded-full border-t-transparent"></div></div>;
  }
      ...
      <aside className="w-64 bg-white border-r border-gray-200 fixed inset-y-0 left-0 z-10 flex flex-col">
        ...
        <div className="p-4 border-t border-gray-100">
          <div className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl p-4 text-white">
            ...
            <p className="text-lg font-bold">{state.wallets.reduce((acc, w) => acc + w.balance, 0).toLocaleString()} SAR</p>
          </div>
        </div>
      </aside>
        ...
        <header className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center gap-3">
             <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-full border shadow-sm">
                <div className="h-6 w-6 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 font-bold text-xs">A</div>
                <span className="text-sm font-medium text-gray-700">AdminL</span>
             </div>
